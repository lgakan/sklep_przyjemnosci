...
import csv
# import sys
# sys.path.append('../sklep_przyjemnosci')
...
parent_percentage = 0.5
chance_to_crossover = 0.7
mutation_type = 'singular'
max_iters = 200
iters_without_change = 30
...
main_client = Client(0, [('item_1', 7),
                         ...
                         ('item_8', 6),
                         ('item_9', 4),
                         ('item_10', 1)], 1200, main_inventory) # medium unique sellers, small database, small items test 1,2,7,8
# main_client = Client(0, [('item_1', 7),
#                          ('item_2', 10),
#                          ('item_5', 2),
#                          ('item_6', 4),
...
#                          ('item_9', 4),
#                          ('item_10', 1),
#                          ('item_17', 2)], 1200, main_inventory) # medium unique sellers, small database, small items test 3,4,6,10,11
# main_client = Client(0, [('item_1', 18),
#                          ('item_7', 18),
#                          ('item_17', 5),
#                          ('item_9', 18)], 1600, main_inventory) #test 9
# main_client = Client(0, [('item_1', 18),
#                          ('item_7', 18),
#                          ('item_9', 18)], 1600, main_inventory)
# main_client = Client(0, [('item_1', 7),
#                          ('item_2', 4),
#                          ('item_3', 3),
#                          ('item_4', 1),
...
gen_pop_funcs = []
for matrix in general_population:
    print(matrix)
    gen_pop_funcs.append(get_penalty_func(ObjFunction(matrix, main_sellers_base, main_inventory), main_client.budget))
print(gen_pop_funcs)
...
def create_report(csv_path: str, ordered_data):
...
def main():
    ...
    return current_best_solution, current_lowest_obj_func, i_iter, best_iter
...
if __name__ == '__main__':
    ...
    #     sol.reset_solution()
    # sol_mat = deepcopy(sol.solution_matrix)
    # create_report("Tests/Test_statistics/test_083.csv", '8.3')
    # for _ in range(30):
    #     create_report("Tests/Test_statistics/test_083.csv", main()[1:])

    # parent_perc = [0.2, 0.3, 0.4, 0.5]
    # for elem in parent_perc:
    #     parent_percentage = elem
    ...
    #                          ('item_12', 9),
    #                          # ('item_14', 14),
    #                          ('item_15', 10)], 1300, main_inventory)
    # sol = Solution(main_sellers_base.sellers_list, main_inventory.product_list)
    # dict_of_sells = main_sellers_base.get_sellers_with_items(main_client.get_product_ids())
    # list_of_orders = main_client.get_oder_quantity()
    # general_population = []
    ...
    # sol_mat = deepcopy(sol.solution_matrix)
    # selection_method = 'roulette'
    # create_report("Tests/Test_statistics/increase_amt_diff_prod.csv", 'few')
    # for _ in range(30):
    #     create_report("Tests/Test_statistics/increase_amt_diff_prod.csv", main()[1:])

    # create_report("Tests/Test_statistics/gen_pop_funcs.csv", 'Client_0')
    # create_report("Tests/Test_statistics/gen_pop_funcs.csv", 'Client_0')
    # for _ in range(30):
    #     create_report("Tests/Test_statistics/increase_amt_diff_prod.csv", main()[1:])
    # # selection_method = 'ranking'
    # pop_size = [10, 20, 50]
    # for elem in pop_size:

    # program wysypuje się dla innego rozmiaru populacji w przypadku ruletki
    # parent_percentage = 0.5
    # selection_method = 'ranking'
    # population_size = 40
    # create_report("Tests/Test_statistics/selection_method.csv", 'roulette')
    # for _ in range(30):
    #     create_report("Tests/Test_statistics/selection_method.csv", main()[1:])
    # chance_list = [0, 0.1, 0.2, 0.5, 0.85, 1]
    #
    # report_list = []
    # # algorithm.max_iters = 300
    # for i in range(len(chance_list)):
    #     create_report("Tests/Test_statistics/new_chance_cross.csv", f'{chance_list[i]}')
    #     chance_to_crossover = chance_list[i]
    #     for _ in range(30):
    #         create_report("Tests/Test_statistics/new_chance_cross.csv", main()[1:])
